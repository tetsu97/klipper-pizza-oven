...
(function () {
  ...
  const GcodeProcessor = {
    generate(state) {
        const metadata = {
            ...
            filament_type: state.filamentType,
            type: state.programType,
            version: 1.0
        };
        let gcodeLines = [
            `; METADATA: ${JSON.stringify(metadata)}`,
            `; Profile generated by Klipper PIZZA Oven`,
            'PROGRAM_CLEAR'
        ];
        if (state.programType === 'annealing') {
            state.segments.forEach(seg => {
                const rampTimeSec = seg.ramp_time * 60;
                const holdTimeSec = seg.hold_time * 60;
                gcodeLines.push(`ADD_SEGMENT TEMP=${seg.temp} RAMP_TIME=${rampTimeSec} HOLD_TIME=${holdTimeSec}`);
            });
        } else {
            const holdTimeSec = state.dryingTime * 60;
            gcodeLines.push(`ADD_SEGMENT TEMP=${state.dryingTemp} RAMP_TIME=1 HOLD_TIME=${holdTimeSec}`);
        }
        gcodeLines.push('EXECUTE_PROGRAM');
        return gcodeLines.join('\n');
    },
    parse(gcode) {
        const state = {
            programName: '', filamentType: '', programType: 'annealing',
            segments: [], dryingTime: 300, dryingTemp: 60
        };
        if (typeof gcode !== 'string') return state; // Safety check
        const metadataLine = gcode.split('\n').find(line => line.startsWith('; METADATA:'));
        if (metadataLine) {
            try {
                const metadata = JSON.parse(metadataLine.replace('; METADATA:', '').trim());
                state.programName = metadata.name || '';
                state.filamentType = metadata.filament_type || '';
                state.programType = metadata.type || 'annealing';
            } catch (e) { console.warn('Could not parse metadata line:', metadataLine); }
        }
        const addSegmentRegex = /ADD_SEGMENT\s+TEMP=([0-9.]+)\s+RAMP_TIME=([0-9.]+)\s+HOLD_TIME=([0-9.]+)/i;
        gcode.split('\n').forEach(line => {
            const match = line.match(addSegmentRegex);
            if (match) {
                state.segments.push({
                    temp: parseFloat(match[1]),
                    ramp_time: Math.round(parseFloat(match[2]) / 60),
                    hold_time: Math.round(parseFloat(match[3]) / 60)
                });
            }
        });
        if (state.programType === 'drying' && state.segments.length > 0) {
            state.dryingTemp = state.segments[0].temp;
            state.dryingTime = state.segments[0].hold_time;
            state.segments = [];
        }
        return state;
    }
        ...
        this.chart = new Chart(ctx, {
            ...
            options:{
                ...
                plugins:{
                    ...
                    tooltip: {
                        backgroundColor: '#2a2a40', titleFont: { size: 14, weight: 'bold' },
                        bodyFont: { size: 12 }, padding: 10, cornerRadius: 8, boxPadding: 4,
                        callbacks: {
  ...
  class SegmentsManager {
    ...
    addRow(defaults={}, triggerChange=true) {
        ...
        const lastTemp = lastRow ? toInt(lastRow.querySelector('.seg-temp').value, 50) : 50;
        const row = document.createElement('div');
        row.className = 'segment-row';
        row.innerHTML = `
            <div class="segment-input-group"><label>Ramp Time (min)<span class="tooltip-container"><i class="tooltip-icon">?</i><span class="tooltip-text">Time in minutes for the oven to reach the target temperature of this segment.</span></span></label><input type="number" class="seg-ramptime" value="${defaults.ramp_time || 60}" min="1" step="1"></div>
            <div class="segment-input-group"><label>Hold Time (min)<span class="tooltip-container"><i class="tooltip-icon">?</i><span class="tooltip-text">Time in minutes for the oven to maintain the target temperature after reaching it.</span></span></label><input type="number" class="seg-holdtime" value="${defaults.hold_time || 0}" min="0" step="1"></div>
            <div class="segment-input-group"><label>Target Temperature (°C)<span class="tooltip-container"><i class="tooltip-icon">?</i><span class="tooltip-text">The temperature in degrees Celsius to be reached in this segment.</span></span></label><input type="number" class="seg-temp" value="${defaults.temp ?? lastTemp}" min="0" step="1"></div>
            <div class="segment-action-group"><button type="button" class="row-del btn btn--danger">X</button></div>`;
        this.container.appendChild(row);
        if (triggerChange) this.onChange?.();
    }
    read(){
        if (!this.container) return [];
        return Array.from(this.container.querySelectorAll('.segment-row')).map(row => ({
            ramp_time: toInt(row.querySelector('.seg-ramptime').value),
            hold_time: toInt(row.querySelector('.seg-holdtime').value),
            temp: toInt(row.querySelector('.seg-temp').value)
        })).filter(s => s.ramp_time > 0);
    }
  }
  ...
  const ProfilesService = {
    ...
    async remove(name){
        const r = await fetch(`/api/gcodes/?name=${encodeURIComponent(name)}`, { method:'DELETE' });
        if (!r.ok) throw new Error(`HTTP ${r.status}: ${await r.text()}`);
    },
  };
  ...
  class ProfileEditorManager {
    ...
    openForCreate() {
        this.state = {
            mode: 'create', programType: 'annealing', programName: '',
            filamentType: '', segments: [], dryingTime: 300, dryingTemp: 60,
        };
    ...
    async openForEdit(name) {
        try {
            ...
            this.state = GcodeProcessor.parse(data.gcode);
            this.state.mode = 'edit';
            this.state.programName = name; 
    ...
    updateChart() {
        ...
        if (this.state.programType === 'annealing') {
            points = [{ time: 0, temp: 25 }];
            let currentTime = 0;
            let lastTemp = 25;
            this.state.segments.forEach(seg => {
                if (seg.temp !== lastTemp) points.push({ time: currentTime, temp: lastTemp });
    ...
    updateUiFromState() {
        ...
        $('#editorModeSwitcher').style.display = (mode === 'create') ? 'flex' : 'none';
    ...
    async save() {
        const programName = (this.state.programName || '').trim();
        if (!programName) {
            return Toast.show('Please enter a valid profile name.', 'info');
        }
        
        showLoadingOverlay(`Saving profile '${programName}'...`);
        const gcode = GcodeProcessor.generate(this.state);
        const payload = {
            name: programName, gcode: gcode, overwrite: true,
            program_name: this.state.programName, filament_type: this.state.filamentType,
  ...
  const ProfilesPage = {
    ...
    async openGcodeEditor(name) {
        try {
            showLoadingOverlay(`Loading G-code for ${name}...`);
            const data = await ProfilesService.getDetails(name);
            this.gcodeEditorCurrentFile = name;
            hideLoadingOverlay();
            
            $('#gcodeEditorTitle').textContent = `Edit G-code: ${name}`;
            const modal = $('#gcodeEditorModal');
            const textarea = $('#gcodeEditorTextarea');
            
            if (this.gcodeEditor) {
                this.gcodeEditor.toTextArea();
                this.gcodeEditor = null;
            }
            ...
            requestAnimationFrame(() => {
                ...
                this.gcodeEditor = window.CodeMirror.fromTextArea(textarea, {
                    ...
                    extraKeys: { 'Ctrl-S': () => this.saveGcodeEditor(), 'Cmd-S': () => this.saveGcodeEditor() }
                });
            ...
            hideLoadingOverlay();
            Toast.show(`Failed to open G-code editor: ${e.message}`, 'error');
        }
    },
    ...
    closeGcodeEditor() {
        ...
        this.gcodeEditorCurrentFile = null;
    },

    async saveGcodeEditor() {
        const name = this.gcodeEditorCurrentFile;
        if (!name || !this.gcodeEditor) return;

        showLoadingOverlay(`Saving ${name}...`);
        const gcode = this.gcodeEditor.getValue();
        const parsedState = GcodeProcessor.parse(gcode);
        
        const payload = {
            name: name, gcode: gcode, overwrite: true,
            program_name: parsedState.programName || name,
            filament_type: parsedState.filamentType,
            mode: parsedState.programType,
        };
        ...
        try {
            ...
            hideLoadingOverlay();
            Toast.show(`Profile '${name}' successfully saved.`, 'success');
            this.closeGcodeEditor();
            await this.loadList();
        } catch (e) {
            hideLoadingOverlay();
            Toast.show(`Save failed: ${e.message}`, 'error');
